...
const titleForNextEdition=(value,contexts)=>{const current=Number(value.match(/제\s*(\d+)\s*회/)?.[1]||0),found=contexts.flatMap(text=>[...text.matchAll(/제\s*(\d+)\s*회/g)].map(match=>Number(match[1]))).filter(number=>number>current);return found.length?value.replace(/제\s*\d+\s*회/,`제${Math.min(...found)}회`):value};
...
for(const fair of [...fairs]){
 ...
 if(titleFound){
  const seriesKey=seriesOf(fair),existing=fairs.filter(item=>seriesOf(item)===seriesKey),latestEnd=existing.map(item=>item.end||item.start||"").sort().at(-1)||"",latestYear=Number(latestEnd.slice(0,4)||0),latestEdition=Math.max(0,...existing.map(item=>Number(item.title.match(/제\s*(\d+)\s*회/)?.[1]||0))),contextEditions=contexts.flatMap(text=>[...text.matchAll(/제\s*(\d+)\s*회/g)].map(match=>Number(match[1]))),numberedEdition=/제\s*\d+\s*회/.test(fair.title),hasNewEdition=contextEditions.some(number=>number>latestEdition),candidates=contextDates.filter(item=>numberedEdition?(hasNewEdition&&item.start>latestEnd):Number(item.start.slice(0,4))>latestYear).sort((a,b)=>a.start.localeCompare(b.start));
  if(candidates[0]){const next=candidates[0],nextYear=next.start.slice(0,4),editionTitle=titleForNextEdition(fair.title,contexts),editionShort=titleForNextEdition(fair.short,contexts),clone={...fair,id:0,title:editionTitle===fair.title?titleForYear(fair.title,nextYear):editionTitle,short:editionShort===fair.short?titleForYear(fair.short,nextYear):editionShort,start:next.start,end:next.end,venue:fair.variableVenue?"세부 행사장 공식 발표 확인":fair.venue,annual:true,seriesKey,verifiedAt:checkedAt.slice(0,10)};fairs.push(clone);discovered.push({seriesKey,title:clone.title,start:clone.start,end:clone.end})}
 }
}
